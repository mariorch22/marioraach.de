...
function toRss(items: Array<{ title: string; link: string; pubDate?: string; description?: string }>) {
...
export async function GET() {
  const posts = (await fetchPosts("de", 100)).filter((p) => (p.category ?? "blog") === "essays");
  const items = posts.map((p) => ({
    title: p.title,
    link: `https://www.marioraach.de/essays/${p.slug}`,

          ...
          <div className="flex-row hidden lg:flex z-50 mix-blend-normal text-white">
            {navbarDaten.map((data, index) => (
              <span className='flex justify-center items-center flex-col pt-4' key={index}>
                ...
                <Link to={data.link} onMouseEnter={() => {setHovered(data.titel)}} onMouseLeave={() => {setHovered("false")}} className="w-full h-10 md:px-1 xl:px-6 my-1.5 font-sans font-semibold text-center flex items-center" style={{fontSize: 35}}>
                  {data.titel}
                </Link>   

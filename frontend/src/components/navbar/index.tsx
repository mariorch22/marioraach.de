...
const Navbar = () => {
    ...
    return(
      <>
        <motion.nav className="fixed w-full z-40 flex justify-between px-4 md:px-8 items-center py-2 h-20 drop-shadow-lg"
          initial="normal" 
          ...
          <div className="flex-row hidden lg:flex z-50 mix-blend-normal text-white">
            {navbarData.map((data, index) => (
              <span className='flex justify-center items-center flex-col pt-4' key={index}>
                <Link to={data.link} onMouseEnter={() => {setHovered(data.titel)}} onMouseLeave={() => {setHovered("false")}} className="w-full h-10 md:px-1 xl:px-6 my-1.5 font-sans font-semibold text-center flex items-center" style={{fontSize: 35}}>{data.titel}</Link>   

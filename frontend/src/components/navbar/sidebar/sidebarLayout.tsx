...
const Sidebar_Layout: React.FC<SidebarProps> = ({ toggle, setToggle }) => {
  ...
  const navbarDatenSidebar: NavbarDataItemSidebar[] = t('navbarDataSidebar', {
    returnObjects: true,
  }) as NavbarDataItemSidebar[];
        ...
        {navbarDatenSidebar.map((link, index) => (
          ...
          >
            <Link
              ...
              className="w-full pl-10 my-1.5 text-6xl font-roboto font-medium"
            >
              {link.titel}
            </Link>
          </motion.div>

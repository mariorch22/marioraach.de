...
const Blogarticle = () => {
  ...
  const { isLoading, isError, data } = useQuery({
    queryKey: ['dataww', blogId],
    queryFn: () => fetchDataDE(blogId as string),
  });
      ...
      <div className="w-full pt-20 lg:pt-32 px-4 xl:px-40 font-inter">
        <section className="relative">
          ...
          <div className="flex justify-center pl-8 pb-8">
            <p className="text-sm lg:text-md flex items-center text-gray-500 font-inter">
          ...
          <div className="xl:px-20 text-xl xl:py-20 rounded-b-3xl font-merriweather leading-relaxed font-semibold">
            {data &&
              data.data &&
              data.data.attributes.a.map((index: any, i: number) => (
                <React.Fragment key={i}>
                  ...
                  {index.text && (
                    <p className="pb-4 tracking-wide font-normal text-base leading-7 font-inter">
                      {useFormatText(index.text)}
                    </p>
                  )}
                  {index.Code && (
                    <CopyBlock
                  ...
                  {index.bild.data && (
                    <span className="w-full flex justify-center items-center">
                      <img
                        className="py-2 w-auto text-center"
                        src={`${MY_URL_STRAPI}${index.bild.data.attributes.url}`}
                        alt="Bild"
                      />
                    </span>
                  )}

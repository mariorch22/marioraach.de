...
const fetchDataDE = async () => {
  const response = await fetch(
    `https://j4hxxa1fo2.execute-api.eu-central-1.amazonaws.com/getMySiteComments`
  );
  if (!response.ok) {
...
const CommentSection: React.FC<CommentSectionProps> = ({ commentState }) => {
  ...
  if (isError) {
    return (
      <p className="w-screen h-screen text-white pt-28 px-40 font-roboto">Error: {error.message}</p>
    );
  }
